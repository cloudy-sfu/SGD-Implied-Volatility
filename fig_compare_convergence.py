...
for beginning_point in np.linspace(0.1, 1, 7):
    with open(f"raw/c-hat/newton-20-{format(beginning_point, '.2f')}", "rb") as f:
        c_hat = pickle.load(f)
        sigma_non_convergence = np.isnan(c_hat[:, -1])
        convergence.append({
    ...
    with open(f"raw/c-hat/adam-3000-{format(beginning_point, '.2f')}", "rb") as f:
        c_hat = pickle.load(f)
        sigma_non_convergence = np.isnan(c_hat[:, -1])
        convergence.append({
            "beginning point": beginning_point,
    ...
    with open(f"raw/c-hat/adabelief-600-{format(beginning_point, '.2f')}", "rb") as f:
        c_hat = pickle.load(f)
        sigma_non_convergence = np.isnan(c_hat[:, -1])
        convergence.append({
            "beginning point": beginning_point,
